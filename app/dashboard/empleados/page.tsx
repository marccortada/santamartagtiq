...
export default function EmpleadosPage() {
  ...
  const assignUidToWorker = useCallback(async (workerId: string, uidRaw: string) => {
    ...
    const res = await fetch(`/api/workers/${encodeURIComponent(workerId)}/cards`, {
      method: 'POST',
    ...
    if (!res.ok) throw new Error(data.error ?? 'Error al asignar tarjeta');
  }, []);
    ...
    try {
      const res = await fetch('/api/workers', {
        ...
        body: JSON.stringify({
          ...
          departamento: newDepartment.trim() || null,
          uid_tarjeta: newCardUid.trim() || null,
        }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setErrorMsg(data.error ?? 'Error al crear empleado');
        return;
      }
      const workerId = data.id as string | undefined;
      if (!workerId) {
  ...
  const saveWorkerProfile = async () => {
    ...
    const res = await fetch(`/api/workers/${encodeURIComponent(selectedWorker.id)}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({
        nombre_completo: editName.trim(),
        departamento: editDepartment.trim() || null,
      }),
    });
    const data = await res.json().catch(() => ({}));
    if (!res.ok) {
      setErrorMsg(data.error ?? 'Error al actualizar empleado');
  ...
  const saveCardUid = async (cardId: string) => {
    ...
    const res = await fetch(`/api/cards/${encodeURIComponent(cardId)}`, {
      ...
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({ uid_fisico: uid }),
    });
    const data = await res.json().catch(() => ({}));
    if (!res.ok) {
      setErrorMsg(data.error ?? 'Error al actualizar tarjeta');
      setSavingCards(false);
      return;
    }
  ...
  const toggleCardActive = async (card: Tarjeta) => {
    ...
    const res = await fetch(`/api/cards/${encodeURIComponent(card.id)}`, {
      ...
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({ activa: !card.activa }),
    });
    const data = await res.json().catch(() => ({}));
    if (!res.ok) {
      setErrorMsg(data.error ?? 'Error al actualizar tarjeta');
  ...
  const toggleWorkerActive = async () => {
    ...
    const res = await fetch(`/api/workers/${encodeURIComponent(selectedWorker.id)}`, {
      ...
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({ activo: !selectedWorker.activo }),
    });
    const data = await res.json().catch(() => ({}));
    if (!res.ok) {
      setErrorMsg(data.error ?? 'Error al actualizar empleado');
