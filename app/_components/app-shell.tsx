...
const navItems: NavItem[] = [
  ...
  },
  {
    ...
    label: 'Ajustes',
    icon: 'A',
    isActive: (pathname) => pathname === '/dashboard/ajustes',
  },
  ...
  useEffect(() => {
    fetch('/api/settings')
      ...
      .then((data) => {
        if (data) {
          setSidebarSettings({
            businessName: data.businessName ?? 'Santa Marta',
            welcomeText: data.welcomeText ?? 'Panel principal',
            terminalName: data.terminalName ?? 'lector-caja-1',
          });
        }
      })
      .catch(() => {});
    ...
    <div className="app-bg">
      <div className="app-shell">
        <aside className="app-sidebar">
          <div>
            <Image
              src="/logo-santa-marta-light.svg"
              alt={sidebarSettings.businessName}
              width={190}
              height={44}
              className="app-logo-image"
              priority
            />
            <p className="app-subtitle">{sidebarSettings.welcomeText}</p>
          </div>
          ...
          <div className="app-device-card">
            Terminal activo
            <br />
            <strong>{sidebarSettings.terminalName}</strong>
          </div>
        </aside>
